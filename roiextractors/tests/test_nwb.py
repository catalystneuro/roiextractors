...
class TestNwb(unittest.TestCase):
    ...
    def test_nwb_writer(self):
        ...
        if os.path.exists(self.saveloc):
            try:
                self.nwbobj = roiextractors.NwbSegmentationExtractor(self.saveloc)
                self.nwbobj.io.close()
                os.remove(self.saveloc)
            except:
                os.remove(self.saveloc)
        try:
            self.simaobj = roiextractors.SimaSegmentationExtractor(self.sima_obj_loc)
        except OSError:
            raise Exception('Could not create sima segmentation object')
        roiextractors.NwbSegmentationExtractor.write_segmentation(self.simaobj, self.saveloc,
                                                                        propertydict=[{'name': 'testvals1',
                                                                                       'description': 'testdesc',
                                                                                       'data': np.arange(3),
                                                                                       'id': self.simaobj.get_roi_ids()[
                                                                                             0:3]},
                                                                                      {'name': 'testvals2',
                                                                                       ...
                                                                                       'id': self.simaobj.get_roi_ids()[
                                                                                             3:6]}],
                                                                        excitation_lambda=500.0, emission_lambda=400.0,
                                                                        nwbfile_kwargs={
                                                                            'session_description': 'nwbfiledesc',
