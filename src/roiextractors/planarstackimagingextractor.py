"""Composition utility that stacks per-plane imaging extractors into a single volumetric extractor."""
...
class PlanarStackImagingExtractor(ImagingExtractor):
    """Class to combine multiple planar ImagingExtractor objects by depth into a single volumetric extractor.

    This is a composition utility, not the canonical base class for volumetric data. Native volumetric
    formats should inherit from ImagingExtractor directly and set ``is_volumetric = True``.
    """
    ...
    def depth_slice(self, start_plane: int | None = None, end_plane: int | None = None):
        """Return a new PlanarStackImagingExtractor ranging from the start_plane to the end_plane."""
        ...
        return DepthSlicePlanarStackImagingExtractor(
            parent_extractor=self, start_plane=start_plane, end_plane=end_plane
        )

    def slice_samples(self, start_sample: int | None = None, end_sample: int | None = None):
        """Return a new PlanarStackImagingExtractor with a subset of samples."""
        raise NotImplementedError(
            "slice_samples is not implemented for PlanarStackImagingExtractor due to conflicts with get_series()."
        )
...
class DepthSlicePlanarStackImagingExtractor(PlanarStackImagingExtractor):
    """Class to get a lazy depth slice.

    This class can only be used for volumetric imaging data.
    Do not use this class directly but use `.depth_slice(...)` on a PlanarStackImagingExtractor object.
    """

    extractor_name = "DepthSlicePlanarStackImagingExtractor"

    def __init__(
        self,
        parent_extractor: PlanarStackImagingExtractor,
        start_plane: int | None = None,
        end_plane: int | None = None,
    ):
        """Initialize a PlanarStackImagingExtractor whose plane(s) subset the parent.

        Subset is exclusive on the right bound, that is, the plane indices of this PlanarStackImagingExtractor range over
        [0, ..., end_plane-start_plane-1].

        Parameters
        ----------
        parent_extractor : PlanarStackImagingExtractor
            The PlanarStackImagingExtractor object to subset the planes of.
