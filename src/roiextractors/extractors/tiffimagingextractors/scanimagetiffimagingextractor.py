...
class ScanImageImagingExtractor(ImagingExtractor):
    ...
    def __init__(
        ...
        self.metadata = read_scanimage_metadata(self.file_path)
        self.parsed_metadata = self.metadata["roiextractors_parsed_metadata"]
        ...
        if self._frames_per_slice > 1:
            raise ValueError("Multiple frames per slice not yet supported")

        # Handle channel selection
        if channel_name is None:
            if self._num_channels > 1:
                warn(
                    f"Multiple channels available: {self._channel_names}. Using the first channel: {self._channel_names[0]}"
                )
            self.channel_name = self._channel_names[0]
        else:
            if channel_name not in self._channel_names:
                raise ValueError(f"Channel name '{channel_name}' not found. Available channels: {self._channel_names}")
            self.channel_name = channel_name

        self.channel_index = self._channel_names.index(self.channel_name)

        # Check if this is a multi-file dataset
        self.file_paths = [self.file_path]
        if "SI.hScan2D.logFramesPerFile" in self.metadata["scan_image_non_varying_frame_metadata"]:
            frames_per_file = self.metadata["scan_image_non_varying_frame_metadata"]["SI.hScan2D.logFramesPerFile"]
            if frames_per_file != float("inf"):
                # This is a multi-file dataset, find other files in the series
        ...
        full_mapping = self._create_sample_to_ifd_mapping(
            ...
            num_acquisition_cycles=self._num_samples,
            num_planes=self._num_planes,
            ifds_per_file=ifds_per_file,
        )

        # Filter mapping for the specified channel
        channel_mask = full_mapping["channel_index"] == self.channel_index
    ...
    def _find_additional_files(self):
        """Find additional files in the series based on the file naming pattern.

        The expected pattern is:
        basename_{acquisition}_001.tif  # First file
        basename_{acquisition}_002.tif  # Second file
        basename_{acquisition}_003.tif  # Third file


        Where 001, 002, 003 the file index.
        """
        ...
        # The last two parts should be the acquisition number and file index
        ...
        all_files = sorted(self.file_path.parent.glob(pattern))

        if len(all_files) > 1:
            self.file_paths = all_files
            print(f"Found {len(all_files)} files in the series.")
