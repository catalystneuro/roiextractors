...
### Fixes
* Fixed `BrukerTiffImagingExtractor` failing on Prairie View 5.7+ recordings whose embedded OME-XML uses `<BinaryOnly>` packaging (a separate `<dataset>.companion.ome` sidecar holds the full OME metadata) instead of an embedded `<Pixels>` block. Construction previously raised `ValueError: No Pixels element found in OME-XML metadata` on these recordings. The extractor now reads structural metadata (file paths, channels, planes, dimension order) from the Bruker configuration XML, which is authoritative across all PrairieView versions. This matches what Bio-Formats' `PrairieReader` does and restores the OME-XML-independent behavior of the deprecated `BrukerTiffSinglePlaneImagingExtractor`. Channel names now come from the `<File channelName="...">` attribute in the Bruker XML, which can differ from the generic `Ch{N}` labels in OME-XML when the user set custom fluorophore labels in PrairieView (e.g. `"Green"`/`"Red"`). [PR #587](https://github.com/catalystneuro/roiextractors/pull/587)
