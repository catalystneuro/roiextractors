...
* `BrukerTiffImagingExtractor` no longer raises `Could not determine sampling frequency` on burst/cycle recordings (Brightness Over Time, multi-cycle timed series) that reset `relativeTime` at each `<Sequence>`. These are not uniformly sampled, so the extractor now exposes the true gapped timeline through `get_timestamps()` (with `has_time_vector()` returning `True`), reports the within-burst frame rate as the nominal `sampling_frequency`, and warns. Continuous single-sequence and volumetric recordings are unaffected. [PR #596](https://github.com/catalystneuro/roiextractors/pull/596)
