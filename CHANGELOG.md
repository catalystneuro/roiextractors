...
### Fixes
* `BrukerTiffImagingExtractor` no longer opens every TIFF file at construction to count its pages. Per-file IFD counts now come from the Bruker configuration XML's `<File page="..."/>` attribute via a new overridable `_get_ifds_per_file()` hook on `MultiTIFFMultiPageExtractor`. This brings construction on large Bruker recordings (70k–470k files) from minutes back to seconds, and lets other vendor subclasses skip the per-file opens whenever they have a faster source of truth. [PR #593](https://github.com/catalystneuro/roiextractors/pull/593)
...
* `BrukerTiffImagingExtractor` no longer raises `Could not determine sampling frequency` on burst/cycle recordings (Brightness Over Time, multi-cycle timed series) that reset `relativeTime` at each `<Sequence>`. These are not uniformly sampled, so the extractor now exposes the true gapped timeline through `get_timestamps()` (with `has_time_vector()` returning `True`) and reports the within-burst frame rate as the nominal `sampling_frequency`. Continuous single-sequence and volumetric recordings are unaffected. [PR #596](https://github.com/catalystneuro/roiextractors/pull/596)
