...
* Fixed `BrukerTiffImagingExtractor` raising `ValueError: Could not determine sampling frequency from Bruker configuration XML` on burst/cycle recordings (Brightness Over Time, and multi-cycle timed series) whose `relativeTime` resets to 0 at the start of each `<Sequence>`. Such recordings are not uniformly sampled, so no single regular rate exists across the gaps between bursts, and the previous code treated that as fatal. The extractor now builds a monotonic global per-frame timeline (offsetting each burst by its real start, taken from `absoluteTime`), reports the within-burst frame rate as `sampling_frequency`, exposes the true gapped timeline through `get_timestamps()` with `has_time_vector()` returning `True`, and warns that the recording is not uniformly sampled. Single-sequence and volumetric recordings, whose `relativeTime` is continuous, are unaffected. This restores the multi-`<Sequence>` handling that the deprecated `BrukerTiffSinglePlaneImagingExtractor` had and that PR #567 dropped. [PR #596](https://github.com/catalystneuro/roiextractors/pull/596)
