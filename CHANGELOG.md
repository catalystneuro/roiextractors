...
### Fixes
* Fixed `OverflowError` in `MultiTIFFMultiPageExtractor` construction on recordings with more than 65,535 files, pages per file, or timepoints by widening the IFD mapping table's index fields from `uint16` to `uint32`. Construction no longer opens every TIFF file up front; per-file IFD counts now come from a new overridable `_get_ifds_per_file()` hook that `BrukerTiffImagingExtractor` overrides to read from the Bruker configuration XML. [PR #591](https://github.com/catalystneuro/roiextractors/pull/591)
