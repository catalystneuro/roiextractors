...
### Fixes
* `MultiTIFFMultiPageExtractor` no longer holds TIFF file handles in instance state. Construction opens each file briefly to read its IFD count and closes it again; `get_series` opens files on demand through a small bounded LRU and closes them before returning. This avoids exhausting the per-process file-descriptor limit on recordings with tens or hundreds of thousands of files, and makes the extractor picklable for multi-processing workflows. [PR #592](https://github.com/catalystneuro/roiextractors/pull/592)
