...
class TestScanImageVolumetricWithFlybackFrames:
    ...
    def test_volumetric_two_channels_multi_files(self):
        """
        ...
        """
        file_path = SCANIMAGE_PATH / "volumetric_two_channels_multi_files" / "vol_two_ch_multi_files_00001_00001.tif"
...
class TestScanImagePlaneIndexParameter:
    ...
    def test_plane_index_parameter_single_channel(self):
        ...
        assert data_plane0.shape == (
            extractor_plane0.get_num_samples(),
            20,
            20,
        ), "Data shape should be (samples, rows, columns)"
        assert data_plane4.shape == (
            extractor_plane4.get_num_samples(),
            20,
            20,
        ), "Data shape should be (samples, rows, columns)"
        assert data_plane8.shape == (
            extractor_plane8.get_num_samples(),
            20,
            20,
        ), "Data shape should be (samples, rows, columns)"
        assert data_full.shape == (
            extractor_full.get_num_samples(),
            20,
            20,
            9,
        ), "Data shape should be (samples, rows, columns, planes)"
    ...
    def test_plane_index_with_slice_sample(self):
        ...
        extractor_slice_sample_0_plane_1 = ScanImageImagingExtractor(
            file_paths=[file_path], channel_name="Channel 4", slice_sample=0, plane_index=1
        )

        extractor_slice_sample_1_plane_0 = ScanImageImagingExtractor(
            file_paths=[file_path], channel_name="Channel 4", slice_sample=1, plane_index=0
        )

        extractor_slice_sample_1_plane_1 = ScanImageImagingExtractor(
            file_paths=[file_path], channel_name="Channel 4", slice_sample=1, plane_index=1
        )

        # Create regular extractors for comparison
        extractor_slice_sample_0 = ScanImageImagingExtractor(
            file_paths=[file_path], channel_name="Channel 4", slice_sample=0
        )

        extractor_slice_sample_1 = ScanImageImagingExtractor(
            file_paths=[file_path], channel_name="Channel 4", slice_sample=1
        )

        # Verify that the extractors with plane_index are not volumetric
        assert (
            extractor_slice_sample_0_plane_0.is_volumetric == False
        ), "Extractor with plane_index should not be volumetric"
        assert (
            extractor_slice_sample_0_plane_1.is_volumetric == False
        ), "Extractor with plane_index should not be volumetric"
        assert (
            extractor_slice_sample_1_plane_0.is_volumetric == False
        ), "Extractor with plane_index should not be volumetric"
        assert (
            extractor_slice_sample_1_plane_1.is_volumetric == False
        ), "Extractor with plane_index should not be volumetric"
        ...
        # Verify that the extractors with plane_index have num_planes=1
        assert (
            extractor_slice_sample_0_plane_0.get_num_planes() == 1
        ), "Extractor with plane_index should have num_planes=1"
        assert (
            extractor_slice_sample_0_plane_1.get_num_planes() == 1
        ), "Extractor with plane_index should have num_planes=1"
        assert (
            extractor_slice_sample_1_plane_0.get_num_planes() == 1
        ), "Extractor with plane_index should have num_planes=1"
        assert (
            extractor_slice_sample_1_plane_1.get_num_planes() == 1
        ), "Extractor with plane_index should have num_planes=1"
        ...
        # Verify the shape of the data
        assert data_slice_sample_0_plane_0.shape == (
            extractor_slice_sample_0_plane_0.get_num_samples(),
            256,
            256,
        ), "Data shape should be (samples, rows, columns)"
        assert data_slice_sample_0_plane_1.shape == (
            extractor_slice_sample_0_plane_1.get_num_samples(),
            256,
            256,
        ), "Data shape should be (samples, rows, columns)"
        assert data_slice_sample_1_plane_0.shape == (
            extractor_slice_sample_1_plane_0.get_num_samples(),
            256,
            256,
        ), "Data shape should be (samples, rows, columns)"
        assert data_slice_sample_1_plane_1.shape == (
            extractor_slice_sample_1_plane_1.get_num_samples(),
            256,
            256,
        ), "Data shape should be (samples, rows, columns)"
        assert data_slice_sample_0.shape == (
            extractor_slice_sample_0.get_num_samples(),
            256,
            256,
            2,
        ), "Data shape should be (samples, rows, columns, planes)"
        assert data_slice_sample_1.shape == (
            extractor_slice_sample_1.get_num_samples(),
            256,
            256,
            2,
        ), "Data shape should be (samples, rows, columns, planes)"
