...
class TestPropertyCopySemantics:
    ...
    copies of the parent's properties. Since set_property() always creates a new _PropertyInfo
    (rebinding the dict key), writes on the child do not affect the parent, and vice versa.
    """
    ...
    def test_property_modification_on_sample_sliced_children_does_not_affect_parent(self):
        ...
        parent.set_property(key="quality", values=np.array([1.0, 2.0, 3.0]), ids=roi_ids)

        sliced = parent.slice_samples(start_sample=0, end_sample=10)
        sliced.set_property(key="quality", values=np.array([9.0, 9.0, 9.0]), ids=roi_ids)
    ...
    def test_property_modification_on_parent_does_not_affect_sample_sliced_children(self):
        ...
        parent.set_property(key="quality", values=np.array([9.0, 9.0, 9.0]), ids=roi_ids)
